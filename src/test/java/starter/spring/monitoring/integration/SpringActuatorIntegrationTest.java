...
public class SpringActuatorIntegrationTest extends AbstractControllerIntegrationTest {
    ...
    @TestFactory
    @DisplayName("[Actuator prometheus] 유효하지 않은 Authentication")
    Stream<DynamicTest> ActuatorPrometheus_whenInvalidAuthentication_thenReturn() {
        /* given */
        String invalidUsername = "invalid";
        String invalidPassword = "password";
        ...
        return Stream.of(
                dynamicTest("유효하지 않은 회원이름, 유효하지 않은 패스워드", () -> {
                    mockMvc.perform(get(ACTUATOR_PROMETHEUS)
                                    .header(AUTHORIZATION, "Basic " + invalidUsernameAndPassword))
                            .andExpect(status().isUnauthorized())
                            .andDo(print());
                }),
    ...
    @Test
    @DisplayName("[Actuator prometheus] 유효한 basic authentication")
    void ActuatorPrometheus_whenValidBasicAuthentication() throws Exception {
        /* given */
